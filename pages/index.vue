  ...
  <div class="h-screen bg-gray-50 dark:bg-slate-800 flex flex-col overflow-hidden">
    ...
    <main class="flex-1 overflow-hidden">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 h-full flex flex-col space-y-6">
        ...
        <!-- lsof Installation Prompt -->
        <div v-if="showLsofInstallPrompt" class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
          <div class="flex items-start">
            ...
            <div class="ml-3 flex-1">
              <h3 class="text-sm font-medium text-yellow-800 dark:text-yellow-200">
                lsof Command Required
              </h3>
              <div class="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
                <p>The <code class="bg-yellow-100 dark:bg-yellow-800 px-1 rounded">lsof</code> command is required to view network connections on this system.</p>
                <div class="mt-3">
                  <p class="font-medium">Installation instructions:</p>
                  <div class="mt-2 space-y-1">
                    <p><strong>macOS:</strong> <code class="bg-yellow-100 dark:bg-yellow-800 px-1 rounded">brew install lsof</code></p>
                    <p><strong>Ubuntu/Debian:</strong> <code class="bg-yellow-100 dark:bg-yellow-800 px-1 rounded">sudo apt install lsof</code></p>
                    <p><strong>CentOS/RHEL:</strong> <code class="bg-yellow-100 dark:bg-yellow-800 px-1 rounded">sudo yum install lsof</code></p>
                    <p><strong>Arch Linux:</strong> <code class="bg-yellow-100 dark:bg-yellow-800 px-1 rounded">sudo pacman -S lsof</code></p>
                  </div>
                </div>
              </div>
              <div class="mt-4 flex space-x-3">
                <button 
                  ...
                  class="text-sm bg-yellow-100 dark:bg-yellow-800 text-yellow-800 dark:text-yellow-200 px-3 py-1 rounded hover:bg-yellow-200 dark:hover:bg-yellow-700 transition-colors"
                >
                  Retry
                </button>
                <button 
                  @click="showLsofInstallPrompt = false"
                  class="text-sm text-yellow-600 dark:text-yellow-400 hover:text-yellow-800 dark:hover:text-yellow-200"
                >
                  Dismiss
                </button>
              </div>
...
onMounted(async () => {
  ...
  try {
    ...
    if (err.message && err.message.startsWith('LSOF_NOT_FOUND:')) {
      // Show lsof installation prompt
      showLsofInstallPrompt.value = true
    }
  }
})
